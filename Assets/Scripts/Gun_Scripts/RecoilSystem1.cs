...
public class RecoilSystem1 : MonoBehaviour
{
    ...
    private IEnumerator StartRecoil()
    {
        ...
        while (TimePassed < duration)
        {
            ...
            Quaternion nextRotation = Quaternion.Euler(CalculateNextRotation());
            Vector3 nextPosition = CalculateNextPosition();

            // Apply to weapon
            transform.localRotation = Quaternion.Lerp(transform.localRotation, nextRotation, AnimationcurveTime);
            transform.localPosition = Vector3.Lerp(transform.localPosition, nextPosition, AnimationcurveTime);

            // Apply similar or adjusted effects to camera if needed
            cameraTransform.localRotation = Quaternion.Lerp(cameraTransform.localRotation, nextRotation, AnimationcurveTime);
            yield return null;
        }
    }
